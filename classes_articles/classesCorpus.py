...
class Corpus:
    ...
    #M méthode de construction de vocabulaires des articles
    
    def get_vocabulaire(self,id_articles):
        """ construction d'un ensemble de  vocabulaires des mots)
        
        Paramètres
        ----------
        id_articles :
              dict qui contient des articles
            
        Retour
        ------
        Aucun
        """
        self.id_articles=id_articles
        vocabilaire = set()
        for cle,article in  self.id_articles.items():
            for voc in article['description'].split(" "):
                vocabilaire.add(voc)
